...
def test_get_reco_success(
    ...
    path = GET_RECO_PATH.format(model_name="test_model", user_id=user_id)
    with client:
        response = client.get(path, headers=get_headers(service_config.token))
    assert response.status_code == HTTPStatus.OK
...
def test_get_reco_userknn_success(
    client: TestClient,
    service_config: ServiceConfig,
) -> None:
    user_id = 176549
    path = GET_RECO_PATH.format(
        model_name="userknn_with_popularity",
        user_id=user_id
    )
    with client:
        response = client.get(path, headers=get_headers(service_config.token))
